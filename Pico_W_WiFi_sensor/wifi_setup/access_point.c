...
*/
#ifndef LOCAL_TEST
    // Ensure we're not in station mode before enabling AP
    cyw43_arch_disable_sta_mode();
    
    printf("Enabling Access Point: '%s'\n", ap_name);
    printf("Authentication: %s\n", password ? "WPA2" : "OPEN (no password)");
    fflush(stdout);
    
    cyw43_arch_enable_ap_mode(ap_name, password, password ? CYW43_AUTH_WPA2_AES_PSK : CYW43_AUTH_OPEN);
    ...
    // Give lwIP/httpd a moment to finish sending the CGI response (e.g. /done.html)
    // before tearing down the access point. Without this, some browsers see a
    // connection error even though settings were saved.
    sleep_ms(1500);
